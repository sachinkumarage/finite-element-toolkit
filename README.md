...
Core dependencies: NumPy (all versions), SciPy (since Version 11, for the generalized eigenvalue solver behind natural-frequency analysis, `scipy.linalg.eigh`), and Matplotlib (since Version 22, for 2D post-processing plots). PyVista (and `imageio`, for GIF export) are **not** core dependencies -- they power only the optional Version 23 3D viewer, and are installed with the `viz3d` extra:

```bash
pip install -e ".[dev,viz3d]"
```
...
- **Version 24** — Engineering GUI and interactive simulation workspace: a Streamlit-based application for project creation, material selection, mesh inspection, boundary-condition setup, solver configuration, interactive result visualization, simulation workflow management, and an engineering dashboard, building directly on Version 22's post-processing framework and Version 23's 3D viewer
